"""Replay of the EXISTING (pre-guarded-band) generation protocol with the ANALYTIC model on the
...
- model: test_model_type='analytic'
  called via TestsGenerator._generate_test -
...
Outputs: analytic_replay_points.csv (all evaluated points),
analytic_replay_qualified_points.csv (restricted to the qualified samples,
with label + test_type + var1/2/3), analytic_replay_tests.csv (one row per
series: generated test or none), analytic_replay_summary.json.

Run from backend/:  PYTHONPATH=. uv run python ../analysis_temp/guarded-band/analytic_replay.py
"""
...
MODEL_TYPE = "analytic"  # trainer/model@model=analytic
...
def main() -> None:
    ...
    points.to_csv(HERE / "analytic_replay_points.csv", index=False)
    tests.to_csv(HERE / "analytic_replay_tests.csv", index=False)
    ...
    qpts.to_csv(HERE / "analytic_replay_qualified_points.csv", index=False)
    ...
    (HERE / "analytic_replay_summary.json").write_text(json.dumps(summary, indent=2))
