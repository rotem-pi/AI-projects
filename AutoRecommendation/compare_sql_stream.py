...
(run unmodified against Postgres) with the insights recorded in Athena.
    ...
    ./venv/bin/python compare_sql_stream.py --task-id 1473514
...
_BACKEND = Path(__file__).parent.parent.parent / "definity-app" / "backend"
...
def _print_report(pg: dict, comparison: dict, athena_rows: list[dict]) -> None:
    ...
    run_context = fetch.get("run_context") or {}
    ...
    sections = {
        name: (len(rows) if isinstance(rows, list) else "1 row")
        for name, rows in run_context.items()
    } if run_context else {}
    print(f"    run_context sections: {sections or 'MISSING'}")
