"""
Frozen copy of `AnalyticTestTimeSeriesModel` from the parent of commit 8f5e1c0
(`git show 8f5e1c0^:backend/app/tests_gen/models/analytical_model.py`).

Used only in regression tests to compare behaviour before vs after 8f5e1c0.
"""
...
from app.tests_gen.models.metric_test_model import TestTimeSeriesModel
...
class LegacyAnalyticHyperParams:
...
legacy_default_hyper_params = asdict(LegacyAnalyticHyperParams())


class LegacyAnalyticTestTimeSeriesModel(TestTimeSeriesModel):
    def __init__(
        self,
        metric_id,
        model_id,
        ...
        hyper_params=legacy_default_hyper_params,
    ):
        ...
        self.hyper_params = LegacyAnalyticHyperParams(**hyper_params)
    ...
    def train(self, metrics, labels):
        ...
        clean, anomalies, iqr = remove_outliers(
            data,
            self.hyper_params.anomaly_lower_bound_p,
            self.hyper_params.anomaly_upper_bound_p,
            self.hyper_params.base_iqr_factor,
        )
        anomaly_ratio = len(anomalies) / max(1, len(clean))
        logger.debug(
            f"Metric {self.metric_id} training: filtered {anomaly_ratio} of training points as anomalous"
        )
        trend_test = self.get_trend(clean)
        pct_test = self.get_pct(data)
    ...
    def filter_tests_by_policy(
        ...
        for tc in candidates:
            ...
            if (
                ...
                or (1 - tc["score"]) * len(metric_history.app_pits)
